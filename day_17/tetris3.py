...
while rocks < ROCKS:
    ...
    if index == 0 and current_shape == order[1] and cycles < 2:
        print(rocks, ':', rocks-last_rocks, ':', highest_point-last_highest, ':', highest_point)
        cycles += 1
        if cycles == 1:
            rock_base = rocks
            y_base = highest_point
        elif cycles == 2:
            rock_diff = rocks - rock_base
            y_diff = highest_point - y_base

            n = (ROCKS - rock_base) // rock_diff
            rocks_n = rock_base + rock_diff * n
            remaining_rocks = ROCKS - rocks_n
    ...
    if moved:
        ...
        # - spawn new rock
        if remaining_rocks is not None:
            remaining_rocks -= 1
...
highest_point += ((n-1) * y_diff)

# 1572093023288 too high
# 1572093023287 too high
# 1572093023286 too high
