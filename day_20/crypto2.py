...
for rep in range(10):
    for val, index in numbers_src:
        ...
        new_index = (i + (val * MULTIPLY) % (numbers_len-1))
        if i == numbers_len-1 and new_index == numbers_len-1:
            pass
        else:
            new_index = new_index % (numbers_len-1)
            if new_index == 0:
...
a = numbers[i_1000][0] * MULTIPLY
b = numbers[i_2000][0] * MULTIPLY
c = numbers[i_3000][0] * MULTIPLY
