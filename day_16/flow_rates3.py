    ...
    if steps == 0:
        ...
        if path['paths'][0]['cooldown'] == 0 and path['paths'][1]['cooldown'] == 0:
            ...
            if len_valves > 8:
                # path_0:
                profit_0 = {}
                position = path['paths'][0]['position']
                tunnels = [(tunnel, length) for tunnel, length in valves_filtered[position]['tunnels'].items() if tunnel in valves]
                for name, length in tunnels:
                    profit_0[name] = (MINUTES - path['length'] - length) * valves_filtered[name]['rate']
                # print(profit_0)
                profit_0f = {}
                while len(profit_0f) < 7:
                    max_ = (None, 0)
                    for name, val in profit_0.items():
                        if val > max_[1]:
                            max_ = (name, val)
                    if max_[0] == None:
                        profit_0f[None] = None
                        break
                    profit_0f[max_[0]] = max_[1]
                    profit_0.pop(max_[0])
                # print(profit_0f)
                # path_1:
                valve_combos = []
                position = path['paths'][1]['position']
                tunnels = [(tunnel, length) for tunnel, length in valves_filtered[position]['tunnels'].items() if tunnel in valves]
                for name_0, val in profit_0f.items():
                    profit_1 = {}
                    for name, length in tunnels:
                        if name != name_0:
                            profit_1[name] = (MINUTES - path['length'] - length) * valves_filtered[name]['rate']
                    # print(profit_1)
                    profit_1f = {}
                    while len(profit_1f) < 7:
                        max_ = (None, 0)
                        for name, val in profit_1.items():
                            if val > max_[1]:
                                max_ = (name, val)
                        if max_[0] == None:
                            profit_1f[None] = None
                            break
                        profit_1f[max_[0]] = max_[1]
                        profit_1.pop(max_[0])
                    # print(profit_1f)
                    for name_1 in profit_1f:
                        valve_combos.append([name_0, name_1])
                # print(valve_combos)
                # exit()
            elif len_valves > 1:
            ...
            for combo in valve_combos:
                ...
                if combo[0] is None and combo[1] is None:
                    ...
                    range_ = 2
                    if path['paths'][0]['position'] == path['paths'][1]['position'] or special:
            ...
            for i in range(2):
                ...
                if path['paths'][i]['cooldown'] == 0:
                    ...
                    if len_valves > 8:
                        ...
                        # profit = {}
                        # position = path['paths'][i]['position']
                        # tunnels_ = [(tunnel, length) for tunnel, length in valves_filtered[position]['tunnels'].items() if tunnel in valves]
                        # for name, length in tunnels_:
                        #     profit[name] = (MINUTES - path['length'] - length) * valves_filtered[name]['rate']
                        # # print(profit_0)
                        # profit_f = {}
                        # while len(profit_f) < 7:
                        #     max_ = (None, 0)
                        ...
                        #         profit_f[None] = None
                        #         break
                        #     profit_f[max_[0]] = max_[1]
                        #     profit.pop(max_[0])
                        # valves_ = set(profit_f.keys())
                        valves_ = copy.deepcopy(valves)
                    else:
                        valves_ = copy.deepcopy(valves)

                    for tunnel, length in tunnels.items():
                        if len(list(valves_)) == 0:
                            stay_still = True
                        elif tunnel not in valves_:
...
# 2423?
# 2480 - too low
#   (after 3658997 paths explored - before shortening)
#   (after 226097 paths explored - after shortening) 5:4:6:5
#   (after 228931 paths explored - after shortening)
# setting 6:5:7:6 -> 2480
# setting 7:6:7:6 -> 2480
# setting 8:7:8:7 -> 2480
